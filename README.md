...
* `Configure Wifi Zones` - You can configure any number of wifi zones, each mapping a wifi SSID to an exact location. Whenever you're connected to one of those networks, the app reports that zone's coordinates instead of the GPS position. This is useful if you don't want to collect a bunch of noisy data when you're at home. Each zone can also set its own `Minimum time between points`, which overrides the global setting while you're connected to that network. For example, a zone for your home wifi can log one point every 5 minutes while everywhere else keeps logging every few seconds. Notes: if two zones somehow have the same SSID, the first one is used; SSIDs are matched case-insensitively; trip settings always take precedence, so a trip started inside a zone still records at the trip's resolution; and zones apply to location updates only, so "Visit" records still contain the real coordinates. Zones can only be configured in the app, not from the server response described below.
